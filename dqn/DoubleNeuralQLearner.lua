...
local dnql = torch.class('dqn.DoubleNeuralQLearner')


function dnql:__init(args)
...
function dnql:preprocess(rawstate)
...
function dnql:getQUpdate(args)
    ...
    -- delta = r + (1-terminal) * gamma * Q'(s2, argmax_a Q(S2,a) ) - Q(s, a)
...
--    q2_max = target_q_net:forward(s2):float():max(2)
    ...
    -- Compute maxA = argmax_a Q(S2,a)
    maxA = select(2, self.network:forward(s2):float():max(2))

    -- Compute q2_max = Q'(s2, argmax_a Q(S2,a) )
    q2_max = target_q_net:forward(s2):float():gather(2, maxA)

    -- Compute q2 = (1-terminal) * gamma * Q'(s2, argmax_a Q(S2,a) )
...
function dnql:qLearnMinibatch()
    -- Perform a minibatch Q-learning update:
    -- w += alpha * (r + gamma * Q'(s2, argmax_a Q(S2,a) ) - Q(s,a)) * dQ(s,a)/dw
...
function dnql:sample_validation_data()
...
function dnql:compute_validation_statistics()
...
function dnql:perceive(reward, rawstate, terminal, testing, testing_ep)
...
function dnql:eGreedy(state, testing_ep)
